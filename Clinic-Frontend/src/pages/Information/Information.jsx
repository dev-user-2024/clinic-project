...
const Information = () => {
      ...
      try {
        ...
        if(category == 0){
          const { data: item } = await axios.get(
            `${serverApi}informationItem?page=${pageNumber}`
          );
          // console.log(item);
          setList(item.data);
          ...
          const { data } = await axios.get(`${serverApi}informationCategory/${category}`);
          // console.log(data);
  ...
  const removeListener = async (id) => {
    ...
    try {
      ...
      const { data } = await axios.delete(`${serverApi}informationItem/${id}`,{
  ...
  return (
    <div>
      ...
      <div className="row mt-4">
        <h6 style={{ color: "#828282" }}>لیست دانستنی ها</h6>
        <div
