...
const LifeSkillsModal = ({ list, setList, categoryList, setCategoruList }) => {
  ...
  const submitForm = async (values) => {
    try {
      ...
      const { data } = await axios.post(`${serverApi}item`, sendData, {
        headers: { Authorization: `Bearer ${token}` },
      });
      // console.log(data);
      const { data2 } = await axios.post(
        `${serverApi}categoryLifeSkills/assignItem/${values.category}`,
        { item_id: data.id } ,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
...
export default LifeSkillsModal;
