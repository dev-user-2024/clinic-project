...
const CategoryModal = ({categoryList,setCategoruList}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { data } = await axios.get(`${serverApi}categoryLifeSkills`);
        if(data.length > 0){
  ...
  const removeListener = async (id) => {
    ...
    try {
      ...
      const { data } = await axios.delete(`${serverApi}categoryLifeSkills/${id}` ,
      {
  ...
  const submitForm = async (values) => {
    try {
      setLoading(true)
      const token = localStorage.getItem("token");
      const { data } = await axios.post(`${serverApi}categoryLifeSkills`, values ,
      {
        headers: { Authorization: `Bearer ${token}` },
      }
      );
